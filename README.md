...
视觉上：标题前是一枚缓慢呼吸的星（主色 `#ff6699`，每 4 秒一个明暗周期）；标题下的分隔线每 3 帧才推进一格，流光从容；进度条、擦除动画同样由该主色展开渐变。**非 UniEditDept 的条目刻意换了一套冷色**：常态高亮为白色，光标行反白，统计占用时整行走蓝紫色扫描动画，与粉红主色一眼区分。动画可用 `-no-anim` 整体关闭。
